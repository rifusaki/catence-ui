...
class MessageBase(ABC):
    ...
    async def remove_children(self):
        """Recursively delete the descendant steps of this message.

        Tool steps and other nested artifacts never enter ``chat_context``, so
        removing a message alone would orphan them: they would survive in the
        data layer (and reappear on reload) while no longer belonging to any
        message. Descendants are collected leaves-first so referential
        integrity constraints are respected, deleted from the data layer, and
        removed from the live UI as well.
        """
        data_layer = get_data_layer()
        if not data_layer:
            return
        ...
        # A malformed cycle back to this message must never cause the message
        # itself to be deleted (edit_message calls this after updating it).
        ordered_descendant_ids = [
            step_id for step_id in collect_descendants(self.id) if step_id != self.id
        ]
        descendant_ids = set(ordered_descendant_ids)

        for step in steps:
            step_id = step.get("id")
            feedback_id = (step.get("feedback") or {}).get("id")
            if step_id in descendant_ids and feedback_id:
                await data_layer.delete_feedback(feedback_id)

        for element in thread.get("elements", []):
            for_id = element.get("forId")
            element_id = element.get("id")
            if for_id in descendant_ids and element_id:
                await data_layer.delete_element(element_id, self.thread_id)

        steps_by_id = {step["id"]: step for step in steps if step.get("id")}
        for descendant_id in ordered_descendant_ids:
            await data_layer.delete_step(descendant_id)
            # Also drop the step from the live chat so nested tool output
            # disappears immediately instead of lingering until a reload.
            await context.emitter.delete_step(steps_by_id[descendant_id])
